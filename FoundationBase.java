...
@Autonomous(name="Foundation Base", group="Base")

public class FoundationBase extends LinearOpMode {
    BNO055IMU imu;
    Orientation angles;
    ...
    public void runOpMode() {
        ...
        sleep(100);
        if (Alliance == Blue){
            EncoderPositionT = robot.TopDrive.getCurrentPosition() - 30 * COUNTS_PER_INCH;
            EncoderPositionB = robot.BottomDrive.getCurrentPosition() + 30 * COUNTS_PER_INCH;
        }
        if (Alliance == Red){
            EncoderPositionT = robot.TopDrive.getCurrentPosition() + 30 * COUNTS_PER_INCH;
            EncoderPositionB = robot.BottomDrive.getCurrentPosition() - 30 * COUNTS_PER_INCH;
        ...
        while (!isStopRequested() && opModeIsActive() && (runtime.seconds() < 15)){
            ...
            telemetry.update();            
            if (Math.abs(robot.TopDrive.getCurrentPosition()-EncoderPositionT) > 12*COUNTS_PER_INCH){
                break;
            }
        }
        robot.leftDrive.setPower(0);
        ...
        while (opModeIsActive()){
            ...
            if (Math.abs(robot.TopDrive.getCurrentPosition() - EncoderPositionT) > 23 * COUNTS_PER_INCH){
                break;
            }
            telemetry.addData("left speed", leftpower);
            telemetry.addData("heading", angles.firstAngle);
